...
なお、実際に今回初めてEcoをプレイする方は、記事内最後の[チュートリアル進行不能バグについて](#チュートリアル進行不能バグについて)を確認しておくことをお勧めします。
...
本ゲームを開始すると画面左上にチュートリアルが表示される。これに従って進めれば本記事で説明した内容は一通り確認できるが、**チュートリアルが進行不能になるバグがある**。具体的には以下を確認しています。
...
> [!warning] チュートリアルを完了しないと経験値が解放されない
> チュートリアル自体はスキップして完了したことにできる(チュートリアル表示欄にカーソルを合わせるとスキップボタンが出てくる)が、**チュートリアルを完了状態にしないと経験値が解放されない**(=専門分野が解放されない)ので、上記のバグに遭遇した場合は注意が必要。
